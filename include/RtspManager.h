...
struct RtspCommand {
  ...
  std::string DESCRIBE = "DESCRIBE rtsp://%s:%lu/%s RTSP/1.0\r\nCSeq: %i\r\nAccept: application/sdp\r\n\r\n";
  ...
  std::string SETUP    = "SETUP rtsp://%s:%lu/%s RTSP/1.0\r\nCSeq: %i\r\nTransport: RTP/AVP/TCP;interleaved=0-1;\r\n\r\n";
  std::string PLAY     = "PLAY rtsp://%s:%lu/%s RTSP/1.0\r\nCSeq: %i\r\nUser-Agent: LibVLC/3.0.14 (LIVE555 Streaming Media v2016.11.28)\r\nSession: %s\r\nRange: npt=0.000-\r\n\r\n";
};
...
public:
  ...
  std::string cmdDescribe(const std::string &host, int port, const std::string &media, int seq);
  std::string cmdSetup(const std::string &host, int port, const std::string &media, int seq);
  std::string getSession(const std::string& setupRes);
  std::string cmdPlay(const std::string &host, int port, const std::string &media, int seq, const std::string &session);
